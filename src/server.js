...
// Security alert system — gracefully no-op when src/alerts/ missing
let alertSystem = null;
try {
  alertSystem = await import("./alerts/index.js");
} catch (err) {
  if (err?.code === "ERR_MODULE_NOT_FOUND" || err?.code === "MODULE_NOT_FOUND") {
    // alerts module not yet built — stubs will no-op
  } else {
    console.error("[alerts] failed to load alerts module:", err?.message || err);
  }
}
...
async function startGateway() {
  ...
  gatewayProc.on("exit", (code, signal) => {
    ...
    if (!shuttingDown) {
      const willRestart = isConfigured();
      const recentLogs = Array.isArray(gatewayLogs) ? gatewayLogs.slice(-200) : [];
      alertSystem?.alertOnGatewayCrash?.({ code, signal, recentLogs, restartDelaySec: willRestart ? 3 : undefined });
    }
    gatewayProc = null;
...
async function ensureGatewayRunning() {
  ...
  if (!gatewayStarting) {
    gatewayStarting = (async () => {
      await startGateway();
      alertSystem?.markGatewayStarted?.();
...
function requireSetupAuth(req, res, next) {
  ...
  if (!isValid) {
    alertSystem?.alertOnAuthFailure?.({ ip, endpoint: req.path });
    res.set("WWW-Authenticate", 'Basic realm="JClaw Setup"');
    ...
    if (ok) {
      ...
      restartGateway()
        .then(() => {
          ...
          alertSystem?.initAlerts?.({ stateDir: STATE_DIR, projectRoot: path.resolve(".") });
        })
...
// Security alert monitoring endpoints
app.get("/setup/api/alerts/status", requireSetupAuth, (_req, res) => {
  if (!alertSystem?.getAlertStatus) {
    return res.json({ ok: true, status: { enabled: false, reason: "alert module not loaded" } });
  }
  return res.json({ ok: true, status: alertSystem.getAlertStatus() });
});

app.post("/setup/api/alerts/rebaseline", requireSetupAuth, async (_req, res) => {
  if (!alertSystem?.rebaseline) {
    return res.status(503).json({ ok: false, error: "alert module not loaded" });
  }
  const result = await alertSystem.rebaseline();
  return res.json({ ok: true, ...result });
});

app.post("/setup/api/alerts/audit", requireSetupAuth, async (_req, res) => {
  if (!alertSystem?.runAudit) {
    return res.status(503).json({ ok: false, error: "alert module not loaded" });
  }
  await alertSystem.runAudit();
  return res.json({ ok: true, message: "Config audit triggered" });
});
